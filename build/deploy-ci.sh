#! /bin/sh
mkdir temp_web
git config --global user.name "urpt-bot"
...
if [ "$TRAVIS_TAG" ]; then
  ...
  cd temp_web
  git clone --depth 1 -b gh-pages --single-branch https://$ROT_TOKEN@github.com/ElemeFE/urpt.git && cd urpt
  # build sub folder
  echo $TRAVIS_TAG
...
mkdir $TRAVIS_BRANCH
rm -rf $TRAVIS_BRANCH/**
cp -rf ../../examples/urpt-ui/** $TRAVIS_BRANCH/
git add -A .
git commit -m "$TRAVIS_COMMIT_MSG"
