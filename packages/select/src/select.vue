...
<script type="text/babel">
  ...
  import scrollIntoView from 'urpt-ui/src/utils/scroll-into-view';
  import { getValueByPath, valueEquals, isIE, isEdge } from 'urpt-ui/src/utils/util';
  ...
  export default {
      ...
      autoComplete: {
        ...
        validator(val) {
          process.env.NODE_ENV !== 'production' &&
            console.warn('[Urpt Warn][Select]\'auto-complete\' property will be deprecated in next major version. please use \'autocomplete\' instead.');
          return true;
        }
      },
    ...
    watch: {
      ...
      visible(val) {
        if (!val) {
          this.broadcast('TcSelectDropdown', 'destroyPopper');
          if (this.$refs.input) {
          ...
          this.broadcast('TcSelectDropdown', 'updatePopper');
          if (this.filterable) {
            ...
            if (this.multiple) {
              ...
              if (!this.remote) {
                this.broadcast('TcOption', 'queryChange', '');
                this.broadcast('TcOptionGroup', 'queryChange');
              }
    ...
    methods: {
      ...
      handleQueryChange(val) {
        ...
        this.previousQuery = val;
        this.$nextTick(() => {
          if (this.visible) this.broadcast('TcSelectDropdown', 'updatePopper');
        });
        this.hoverIndex = -1;
        ...
        if (this.remote && typeof this.remoteMethod === 'function') {
          ...
          this.broadcast('TcOption', 'queryChange', val);
          this.broadcast('TcOptionGroup', 'queryChange');
        }
        if (this.defaultFirstOption && (this.filterable || this.remote) && this.filteredOptionsCount) {
          this.checkDefaultFirstOption();
